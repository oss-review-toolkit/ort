            ...
            if "platform" in manifest and manifest["platform"]["architecture"] in [
                ...
                if "layers" in layer_manifest:
                    for layer in layer_manifest["layers"]:
                        if layer["digest"] not in exclusion_list:
                            exclusion_list.append(layer["digest"])
                        print(
                            f":locked: Added digest to exclusion list {layer['digest']}"
                        )
...
def delete_packages():
    ...
    for package in packages:
        ...
        for page in range(pages, 0, -1):
            ...
            if len(images) > keep:
                for image in images if page != 1 else images[keep:]:
                    url = f"{pkg_url}/container/{package.replace('/', '%2F')}/versions/{image['id']}"

                    # Never remove latest or non snapshot tagged images if not skipt stated
                    if not ignore_skip:
