...
RUN syft / --exclude '*/usr/share/doc' --exclude '*/etc' -o spdx-json=/usr/share/doc/ort/ort-base.spdx.json

# Python
ENV PYENV_ROOT=/opt/python
ENV PATH=$PATH:$PYENV_ROOT/shims:$PYENV_ROOT/bin
COPY --from=python --chown=$USER:$USER $PYENV_ROOT $PYENV_ROOT
RUN syft $PYENV_ROOT -o spdx-json=/usr/share/doc/ort/ort-python.spdx.json
...
RUN syft $NVM_DIR  -o spdx-json=/usr/share/doc/ort/ort-nodejs.spdx.json
...
RUN syft $RUST_HOME -o spdx-json=/usr/share/doc/ort/ort-rust.spdx.json

# Golang
ENV PATH=$PATH:/opt/go/bin
COPY --from=golang --chown=$USER:$USER /opt/go /opt/go
RUN syft /opt/go -o spdx-json=/usr/share/doc/ort/ort-golang.spdx.json
...
RUN syft $RBENV_ROOT -o spdx-json=/usr/share/doc/ort/ort-ruby.spdx.json
...
RUN syft $ANDROID_HOME -o spdx-json=/usr/share/doc/ort/ort-android.spdx.json
...
RUN syft $SWIFT_HOME -o spdx-json=/usr/share/doc/ort/ort-swift.spdx.json
...
RUN syft $SBT_HOME -o spdx-json=/usr/share/doc/ort/ort-sbt.spdx.json
...
RUN syft $DART_SDK -o spdx-json=/usr/share/doc/ort/ort-golang.dart.json
...
RUN syft $DOTNET_HOME -o spdx-json=/usr/share/doc/ort/ort-dotnet.spdx.json
...
RUN syft /opt/php -o spdx-json=/usr/share/doc/ort/ort-php.spdx.json
...
RUN syft /opt/haskell -o spdx-json=/usr/share/doc/ort/ort-haskell.spdx.json
