...
The OSS Review Toolkit (ORT) assists with verifying Free and Open Source Software license compliance by checking project
source code and its dependencies.
...
The following tools are [planned](https://github.com/heremaps/oss-review-toolkit/projects/1) by not yet available:
...
Run `docker/run.sh <DOCKER_ARGS> <ORT_ARGS>` where `<DOCKER_ARGS>` are passed to `docker run` and `<ORT_ARGS>` are
passed to ORT. You typically use `<DOCKER_ARGS>` to mount the project directory to scan into the running container to
let ORT access it, for example:
...
A basic ORT pipeline (using the analyzer, scanner and reporter) can be easily run on [Jenkins CI](https://jenkins.io/)
by using the [Jenkinsfile](./Jenkinsfile) in a (declarative) pipeline job.
