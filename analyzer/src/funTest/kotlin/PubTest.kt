    ...
    init {
        ...
        "Pub" should {
            "resolve dart http dependencies correctly" {
                val workingDir = File(projectsDirExternal, "http")
                val packageFile = File(workingDir, "pubspec.yaml")
                var expectedResultFile = File(projectsDirExternal, "http-expected-output.yml");

                val result = createPubForExternal().resolveDependencies(listOf(packageFile))[packageFile]
                result!!.errors.size shouldBe 0

                //expectedResultFile.writeText(yamlMapper.writeValueAsString(result));
            ...
            "Resolve dependencies for a project with flutter correctly" {
                ...
                val packageFile = File(workingDir, "pubspec.yaml")
                var expectedResultFile = File(projectsDir.parentFile, "pub-expected-output-project-with-flutter.yml");
            ...
            "Resolve dependencies for a project with dependencies without a static version" {
                ...
                val packageFile = File(workingDir, "pubspec.yaml")
                var expectedResultFile = File(projectsDir.parentFile, "pub-expected-output-any-version.yml");
