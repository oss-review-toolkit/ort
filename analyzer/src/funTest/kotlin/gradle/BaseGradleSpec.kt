    ...
    init {
        ...
        "analyzer results for all .gradle files match expected"{
            ...
            val testRows = analyzerResultsDir.walkTopDown().asIterable().filter { file: File ->
                ...
                val fileExpectedResultPath = expectedResultsDir + it.path.substringBeforeLast(File.separator).substringAfterLast("analyzer_results").replace("\\", "/") + "/" + it.name //keep as unix paths
