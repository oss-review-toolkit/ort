...
class DrupalIntegrationTest : AbstractIntegrationSpec() {
    ...
    override val definitionFilesForTest by lazy {
        val phpComposerFiles: List<File> = expectedDefinitionFiles[PhpComposer]?.filter {
            it.readText().isNotBlank()
        } as List<File>
        mapOf(
                PhpComposer as PackageManagerFactory<PackageManager> to phpComposerFiles
        )
    }
}
