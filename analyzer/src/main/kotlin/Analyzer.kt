...
class Analyzer(private val config: AnalyzerConfiguration) {
    fun analyze(
        ...
        val nestedVcs = workingTree?.getNested()?.filter { (path, _) ->
            // Only include nested VCS if they are part of the analyzed directory. Do not check this if the VCS type is
            // "GitRepo" because in this case the path is not used for sparse checkout but points ot the manifest file.
            vcs.type == "GitRepo" || path.startsWith(vcs.path)
        } ?: emptyMap()
