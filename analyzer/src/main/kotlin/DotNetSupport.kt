...
class DotNetSupport(
    ...
    init {
        for (entry: Map.Entry<String, String> in packageReferencesMap) {
    ...
    private fun getRightVersionUrl(dataIterator: (MutableIterator<JsonNode>)?,
    ...
    private fun getFirstMatchingIdUrl(dataIterator: MutableIterator<JsonNode>?, packageID: String): String? {
        while (dataIterator != null && dataIterator.hasNext()) {
            val packageNode = dataIterator.next()
            if (packageNode["id"].textValueOrEmpty() == packageID) {
