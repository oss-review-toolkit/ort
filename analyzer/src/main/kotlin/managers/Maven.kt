...
object Maven : PackageManager(
    ...
    override fun prepareResolution(definitionFiles: List<File>) {
        ...
        projectBuildingResults.forEach { projectBuildingResult ->
            ...
            log.debug("Building model for '${projectBuildingResult.pomFile.absolutePath}'.")
        ...
        return DefaultPlexusContainer(configuration).apply {
            loggerManager = object : BaseLoggerManager() {
                override fun createLogger(name: String): Logger {
                    return MavenLogger(log.effectiveLevel)
                }
            }
        }
    }
    ...
    private fun parseVcsInfo(mavenProject: MavenProject): VcsInfo {
        return VcsInfo(parseVcsProvider(mavenProject), parseVcsUrl(mavenProject), parseVcsRevision(mavenProject))
    }
    ...
    private data class VcsInfo(val provider: String, val url: String, val revision: String) {
        fun isEmpty(): Boolean {
            return provider.isEmpty() && url.isEmpty() && revision.isEmpty()
        }
    }
