...
class PIP(name: String, analyzerConfig: AnalyzerConfiguration, repoConfig: RepositoryConfiguration) :
    ...
    private fun getBinaryArtifact(pkg: Package, artifacts: ArrayNode): RemoteArtifact {
        // Prefer python wheels and fall back to the first entry (probably a sdist).
        val binaryArtifact = artifacts.asSequence().find {
    ...
    private fun getSourceArtifact(artifacts: ArrayNode): RemoteArtifact {
        val sourceArtifacts = artifacts.asSequence().filter {
