...
class PhpComposer : PackageManager() {
    ...
    override fun resolveDependencies(definitionFile: File): ProjectAnalyzerResult? {
        ...
        try {
            ...
            val (packageIdentifier, declaredLicenses, _, projectHomepageUrl, _, _, _, _) =
            ...
            val project = Project(
                    id = Identifier(
                            ...
                            namespace = packageIdentifier.namespace,
        ...
        return try {
            ...
            dependencies.addAll(dependenciesLines.map {
                ...
                PackageReference(
                        id = Identifier(
                                provider = PhpComposer.toString(),
                                namespace = scopeDependencyPkgName.substringBefore("/"),
    ...
    private fun parseSourceArtifact(dist: String): RemoteArtifact {
        ...
        return if (url != null) {
            val algorithm = if (hash != null && hash.isNotBlank()) "sha1" else ""
            RemoteArtifact(url, hash ?: "", HashAlgorithm.fromString(algorithm))
        } else {
