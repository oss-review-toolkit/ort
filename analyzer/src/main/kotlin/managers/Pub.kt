...
class Pub(
    ...
    /**
     * A workspace reader that is backed by the local .pub-cache folder.
     */
    private class PubCacheReader {
        // TODO: If there is a .pub-cache directory in Flutter root, 'flutter packages get' will use that one instead of
        //       the .pub-cache directory in user home. Currently we do not support usage of two different pub-cache
        //       directories. Therefore you need to delete the .pub-cache directory in your flutter root folder to force
        //       flutter to use the default one.
    ...
    private fun scanAndroidPackages(packageInfo: JsonNode): ProjectAnalyzerResult? {
        ...
        return if (analyzerResultCacheAndroid.containsKey(packageName)) {
            ...
            Gradle("Gradle", projectDir, analyzerConfig, repoConfig)
