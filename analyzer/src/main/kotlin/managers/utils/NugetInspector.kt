...
internal object NugetInspector : CommandLineTool, Logging {
    override fun command(workingDir: File?) = "/opt/nuget-inspector/nuget-inspector"
...
private fun NugetInspector.ParentDep.toPackageReference() =
    PackageReference(
        id = Identifier(type = TYPE, namespace = "", name = parent.name, version = parent.version),
        dependencies = dependencies.mapTo(sortedSetOf()){
            PackageReference(
                id = Identifier(type = TYPE, namespace = "", name = it.name, version = it.version),
            )
        }
    )
