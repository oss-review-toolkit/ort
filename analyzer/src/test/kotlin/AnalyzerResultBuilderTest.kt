    ...
    init {
        ...
        "AnalyzerResultBuilder" should {
            ...
            "apply scope includes" {
                val scopeIncludes = ScopeInclude("scope-1", ScopeIncludeReason.SOURCE_OF)
                val includes = Includes(scopes = listOf(scopeIncludes))
            ...
            "apply scope includes and excludes" {
                val scopeIncludes = ScopeInclude("scope-1", ScopeIncludeReason.SOURCE_OF)
                val scopeExcludes = ScopeExclude("scope-1", ScopeExcludeReason.DOCUMENTATION_DEPENDENCY_OF)
                val includes = Includes(scopes = listOf(scopeIncludes))
                val excludes = Excludes(scopes = listOf(scopeExcludes))
