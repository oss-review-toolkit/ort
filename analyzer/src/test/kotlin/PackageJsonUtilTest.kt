...
import managers.PackageJsonUtil
import managers.PackageJsonUtil.Companion.mapDefinitionFilesForNpm
import managers.PackageJsonUtil.Companion.mapDefinitionFilesForYarn
...
class PackageJsonUtilTest() : StringSpec() {
    ...
    init {
        ...
        "given project with no lockfile, definition file is mapped for npm only" {
            ...
            mapDefinitionFilesForNpm(definitionFiles) shouldContainExactly
                    absolutePaths("a/package.json")
    ...
    private fun setupProject(path: String, matchers: List<String> = emptyList(),
                             hasNpmLockFile: Boolean = false, hasYarnLockFile: Boolean = false) {
