...
internal class ImportScanResultsCommand : OrtHelperCommand(
    ...
    override fun run() {
        ...
        ortResult.getIdentifiers().forEach { id ->
            val vcsPath = ortResult.getPackageOrProject(id)?.metadata?.vcsProcessed?.path.orEmpty()

            ortResult.getScanResultsForId(id).forEach { scanResult ->
                val resultToStore = if (vcsPath.isNotEmpty()) scanResult.filterByPath(vcsPath) else scanResult
                scanResultsStorage.add(id, resultToStore)
            }
        }
    }
}
