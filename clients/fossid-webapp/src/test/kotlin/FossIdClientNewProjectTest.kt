...
class FossIdClientNewProjectTest : StringSpec({
    val wiremock = WireMockServer(
        WireMockConfiguration.options()
            .dynamicPort()
            .usingFilesUnderDirectory("src/test/assets/test1/")
    )
    ...
    "Version can be extracted from index" {
        service.getLoginPage().execute().body().shouldNotBeNull {
            this.string().shouldContain("cli.  3.1.16 (build 5634934d, RELEASE)")
        }
    }
    ...
    "Marked files can be listed when there are none" {
        service.listMarkedAsIdentifiedFiles("", "", SCAN_CODE).execute().body().shouldNotBeNull {
            ...
            val files = toList(MarkedAsIdentifiedFile::class)
            files.shouldBeEmpty()
        }
    }
