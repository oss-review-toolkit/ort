...
In order to scan the source code of dependencies, ORT relies on the metadata provided by those packages. Often the
metadata contains information how to locate the source code, but this is not always the case. Often the metadata of
packages has no VCS information, points to outdated repositories, or the repositories are not correctly tagged. Because
this information can not always be fixed in remote packages ORT provides a mechanism to curate metadata of packages.
