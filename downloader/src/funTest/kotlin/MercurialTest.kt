...
const val repoUrl = "https://bitbucket.org/creaceed/mercurial-xcode-plugin"
...
class MercurialTest : StringSpec() {
    ...
    init {
        "Mercurial can download entire repo" {
            Mercurial.download(repoUrl, null, null, "", outputDir)
        ...
        "Mercurial can download single revision" {
            ...
            Mercurial.download(repoUrl, revision, null, "", outputDir)
            Mercurial.getWorkingDirectory(outputDir).getRevision() shouldBe revision
        }.config(tags = setOf(Expensive))

        "Mercurial can download sub path" {
            val subdir = "Classes"
            val notCheckoutSubDir = "Resources"
            Mercurial.download(repoUrl, null, subdir, "", outputDir)
            val outputDirList = Mercurial.getWorkingDirectory(outputDir).workingDir.list()
            outputDirList.indexOf(subdir) shouldBe beGreaterThan(-1)
            outputDirList.indexOf(notCheckoutSubDir) shouldBe -1
        }.config(tags = setOf(Expensive))

        "Mercurial can download version" {
            val version = "1.1"
            val revision1_1 = "562fed42b4f3"
            Mercurial.download(repoUrl, null, null, version, outputDir)
            Mercurial.getWorkingDirectory(outputDir).getRevision() shouldBe revision1_1
        }.config(tags = setOf(Expensive))
    }
}
