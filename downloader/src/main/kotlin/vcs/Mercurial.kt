...
object Mercurial : VersionControlSystem() {
    ...
    private val extensionSparse = "sparse = "
    private val extensionLargeFiles = "largefiles = "
    ...
    override fun getWorkingDirectory(vcsDirectory: File) =
            object : WorkingDirectory(vcsDirectory) {
                override fun isValid(): Boolean {
                    ...
                    return workingDir.path.contains(
                            repositoryRoot.stdout().trim())
                }

                override fun getRemoteUrl() = runMercurialCommand(workingDir, "paths",
                                                                  "default").stdout().trimEnd()
                ...
                override fun getPathToRoot(path: File): String {
                    ...
                    val root = getRootPath(absolutePath);
                    return absolutePath.absolutePath.replace(root, "").trimStart('/')
                }
            }

    override fun isApplicableProvider(vcsProvider: String) = vcsProvider.equals("mercurial", true)

    override fun isApplicableUrl(vcsUrl: String) = ProcessCapture(null, "hg", "identify", vcsUrl).exitValue() == 0
    ...
    private fun runMercurialCommand(workingDir: File, vararg args: String): ProcessCapture {
        val processCapture = ProcessCapture(workingDir, "hg", *args).requireSuccess()
