...
class RuleSet(val ortResult: OrtResult) {
     ...
     */
    fun packageRule(name: String, block: PackageRule.() -> Unit) {
        ...
        packages.forEach { pkg ->
            ...
            rule.block()
            if (rule.run() && rule.issues.isNotEmpty()) {
                println(
                    "\tFound issues:\n\t\t${rule.issues.joinToString("\n\t\t") { "${it.severity}: ${it.message}" }}"
                )
            }
        }
    }
     ...
     */
    fun dependencyRule(name: String, block: DependencyRule.() -> Unit) {
        fun traverse(
            pkgRef: PackageReference,
            parents: List<PackageReference>,
            level: Int,
            scope: Scope,
            project: Project
        ) {
            ...
            val rule = DependencyRule(this, name, pkg, detectedLicenses, pkgRef, parents, level, scope, project)
            ...
            pkgRef.dependencies.forEach { dependency ->
                traverse(dependency, listOf(pkgRef) + parents, level + 1, scope, project)
            }
        }
        ...
        ortResult.analyzer?.result?.projects?.forEach { project ->
            project.scopes.forEach { scope ->
                scope.dependencies.forEach { pkgRef ->
                    traverse(pkgRef, emptyList(), 1, scope, project)
                }
            }
        }
    }
 ...
 */
fun ruleSet(ortResult: OrtResult, block: RuleSet.() -> Unit) {
