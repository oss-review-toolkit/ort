...
// compatibility with all licenses involved in a project, i.e. whether code can be combined under several licenses in
// one new project, independently of the outbound license. This covers the use-case of publishing a proprietary project
// with Open Source dependencies. To perform the check, it is necessary to check the compatibility of all licenses among
// each other [2].
//
...
// Use this rule like:
//
// $ ort evaluate -i scanner/src/funTest/assets/scanner-integration-all-pkgs-expected-ort-result.yml --rules-resource /rules/osadl/all-license.rules.kts
...
val ruleSet = ruleSet(ortResult, licenseInfoResolver) {
    ...
    val allLicensesByProject = mutableMapOf<Identifier, MutableMap<SpdxSingleLicenseExpression, MutableSet<Identifier>>>()

    // Define a rule that is executed for each dependency.
    dependencyRule("OSADL_ALL_LICENSE_DEPENDENCY_RULE") {
        ...
        val allProjectLicenses = allLicensesByProject.getOrPut(project.id) { mutableMapOf() }

        outboundLicenses.forEach { outboundLicense ->
            allProjectLicenses.getOrPut(outboundLicense) { mutableSetOf() } += project.id
        }
    ...
    val allLicenses = mutableMapOf<SpdxSingleLicenseExpression, MutableSet<CuratedPackage>>()

    packageRule("OSADL_ALL_PACKAGE_RULE") {
        licenseRule("OSADL_ALL_LICENSE_RULE", licenseView) {
            allLicenses.getOrPut(license) { mutableSetOf() } += pkg
        }
    }
