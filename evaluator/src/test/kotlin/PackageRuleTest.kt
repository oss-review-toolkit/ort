    ...
    init {
        "hasLicense()" should {
            "return true if the package has concluded licenses" {
                val rule = PackageRule(ruleSet, "test", packageWithOnlyConcludedLicense, emptyList())
            ...
            "return true if the package has declared licenses" {
                val rule = PackageRule(ruleSet, "test", packageWithOnlyDeclaredLicense, emptyList())
                val matcher = rule.hasLicense()
            ...
            "return true if the package has detected licenses" {
                val rule = PackageRule(ruleSet, "test", packageWithoutLicense, licenseFindings)
                val matcher = rule.hasLicense()
            ...
            "return false if the package has no license" {
                val rule = PackageRule(ruleSet, "test", packageWithoutLicense, emptyList())
                val matcher = rule.hasLicense()
        ...
        "isExcluded()" should {
            "return true if the package is excluded" {
                val rule = PackageRule(ruleSet, "test", packageExcluded, emptyList())
        ...
        "isSpdxLicense()" should {
            "return true if the license is an SPDX license" {
                PackageRule(ruleSet, "test", packageWithoutLicense, emptyList()).apply {
