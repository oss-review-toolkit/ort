...
class MergeAnalyzerResultsCommand : CliktCommand(
    ...
    private fun mergeIssues(
        ...
        right.forEach { (identifier, issuesList) ->
            ...
            result[identifier]?.let { mergedList.addAll(it) }
            mergedList.addAll(issuesList)

            result[identifier] = mergedList
        }
    ...
    private fun mergeScopes(leftScopes: SortedSet<Scope>, rightScopes: SortedSet<Scope>): SortedSet<Scope> {
        ...
        leftScopes.forEach { scope -> scope.also { scopeMap[scope.name] = it } }
            ...
            scopeMap.merge(
                ...
                Scope(
                    ...
                    dependencies = mergeSortedSets(
                        flattenDependencies(left.dependencies),
                        flattenDependencies(right.dependencies)
                    )
                )
            }
        }
    ...
    private fun flattenDependencies(dependencyTrees: SortedSet<PackageReference>): SortedSet<PackageReference> {
