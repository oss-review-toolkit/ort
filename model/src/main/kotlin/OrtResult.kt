...
data class OrtResult(
    ...
    fun isExcluded(id: Identifier): Boolean
        = getExcludes().isExcluded(id, this)
    ...
    private fun getPackages(): Set<CuratedPackage>
            = analyzer?.result?.packages ?: emptySet()

    private fun getProjects(): Set<Project>
            = analyzer?.result?.projects ?: emptySet()

    private fun getExcludes(): Excludes
            = repository.config.excludes ?: Excludes()
}
