...
class Includes(
    ...
    companion object {
        /**
         * A constant for an [Includes] instance that does not contain any includes. Therefore, using this instance
         * does not include any elements, but also it does not consider includes as present and does not exclude
    ...
    /**
     * True if any [path include][paths] matches [path].
     */
    fun isPathIncluded(path: String) = paths.any { it.matches(path) }
}
