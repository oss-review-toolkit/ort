...
class DependencyGraphBuilder<D>(
    ...
    private fun checkReferences() {
        ...
        val packageReferencesKeysWithMultipleDistintPackageReferences = directDependencies.groupBy { it.key }.filter {
            it.value.distinct().size > 1
        }.map { it.key }

        require(packageReferencesKeysWithMultipleDistintPackageReferences.isEmpty()) {
            "Found multiple distinct package references for each of the following package / fragment index tuples " +
                "${packageReferencesKeysWithMultipleDistintPackageReferences.joinToString()}."
        }
    }
...
private fun Collection<DependencyReference>.toGraph(): Pair<List<DependencyGraphNode>, List<DependencyGraphEdge>> {
    ...
    val nodeIndexMapping = mutableMapOf<DependencyReferenceKey, Int>()

    fun getOrAddNodeIndex(ref: DependencyReference): Int {
        if (!nodeIndexMapping.containsKey(ref.key)) {
            nodeIndexMapping[ref.key] = nodes.size
            nodes += DependencyGraphNode(ref.pkg, ref.fragment, ref.linkage, ref.issues)
        }

        return nodeIndexMapping.getValue(ref.key)
    }
