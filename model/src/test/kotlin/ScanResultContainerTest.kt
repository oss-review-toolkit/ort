...
class ScanResultContainerTest : WordSpec() {
    ...
    private val error11 = OrtError(source = "source-11", message = "error-11")
    private val error12 = OrtError(source = "source-12", message = "error-12")
    private val error21 = OrtError(source = "source-21", message = "error-21")
    private val error22 = OrtError(source = "source-22", message = "error-22")
        ...
        "ScanResults" should {
            ...
            "deprecated errors field in scan summary can be parsed" {
                ...
                scanResults.results[0].summary.errors shouldBe listOf(
                        OrtError(timestamp = Instant.EPOCH, source = "", message = "error-11"),
                        OrtError(timestamp = Instant.EPOCH, source = "", message = "error-12")
                )

                scanResults.results[1].summary.errors shouldBe listOf(
                        OrtError(timestamp = Instant.EPOCH, source = "", message = "error-21"),
                        OrtError(timestamp = Instant.EPOCH, source = "", message = "error-22")
                )
            }
        }
    }
