...
class IncludesTest : WordSpec() {
    ...
    private val pkg = CuratedPackage(metadata = org.ossreviewtoolkit.model.Package.EMPTY.copy(id = id))
    ...
    init {
        "isExcluded" should {
            "return false if the package is not included but no includes are defined" {
                setProjects(
        ...
        "isPackageExcluded" should {
            ...
            "return false if the package is not excluded nor included" {
                setProjects(
                    project1.copy(scopeDependencies = setOf(scope1))
                )
