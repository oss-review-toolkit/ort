    ...
    init {
        "SpdxExpressionLexer" should {
            "create the correct tokens for a valid expression" {
                val testExpression = "(license-1 AND OR license.2 WITH 0LiCeNsE-3+) and (or with license-4 license-.5(("

                val charStream = CharStreams.fromString(testExpression)
                val lexer = SpdxExpressionLexer(charStream)
                val tokens = lexer.allTokens.map { it.type to it.text }
            ...
            "fail for an invalid expression" {
                ...
                val exception = shouldThrow<Error> {
                    lexer.allTokens.map { it.type to it.text }
                }
