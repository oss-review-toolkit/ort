...
class PurlExtensionsTest : WordSpec({
    ...
    "When mapping to Identifier Purl representation" should {
        ...
        "properly parse purls from purl spec examples" {
            ...
            val identifiers = purls.map { it.toIdentifier() }

            identifiers[0] shouldBe Identifier("deb", "debian", "curl", "7.50.3-1")
            identifiers[1] shouldBe Identifier("gem", "", "jruby-launcher", "1.1.2")
            identifiers[2] shouldBe Identifier("golang", "google.golang.org", "genproto", "")
            identifiers[3] shouldBe Identifier("maven", "org.apache.xmlgraphics", "batik-anim", "1.9.1")
            identifiers[4] shouldBe Identifier("npm", "@angular", "animation", "12.3.1")
            identifiers[5] shouldBe Identifier("nuget", "", "EnterpriseLibrary.Common", "6.0.1304")
            identifiers[6] shouldBe Identifier("pypi", "", "django", "1.11.1")
            identifiers[7] shouldBe Identifier("rpm", "fedora", "curl", "7.50.3-1.fc25")
            identifiers[8] shouldBe Identifier("rpm", "opensuse", "curl", "7.56.1-1.1.")
        }
    }
})
