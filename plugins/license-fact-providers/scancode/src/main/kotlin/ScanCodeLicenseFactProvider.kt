...
private const val FALLBACK_DIR = "/opt/scancode-license-data"
...
class ScanCodeLicenseFactProvider(
    ...
    private val scanCodeLicenseTextDir: File by lazy {
        if (config.scanCodeLicenseTextDir != null) {
            File(config.scanCodeLicenseTextDir).takeIf { it.isDirectory }.also {
                requireNotNull(it) {
        ...
        File(FALLBACK_DIR).takeIf { it.isDirectory }.also {
            if (it == null) {
    ...
    private fun getLicenseTextFile(licenseId: String): File? {
        val filename = "${licenseId.removePrefix("LicenseRef-").lowercase()}.LICENSE"
