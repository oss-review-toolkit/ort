...
import org.ossreviewtoolkit.plugins.packagecurationproviders.ortbomfile.OrtBomFileCurationProvider
import org.ossreviewtoolkit.utils.common.div
import org.ossreviewtoolkit.utils.common.extractResource

class OrtBomFilePackageCurationProviderTest : StringSpec() {
    private val curationsFile = extractResource("/ort-bom.json", tempfile(suffix = ".json"))

    init {
        "Provider can read JSON file" {
            val provider = OrtBomFileCurationProvider(curationsFile)

            provider.packageCurations should haveSize(2)
        }
        ...
        "Provider throws an exception if the curations file is not de-serializable" {
            ...
            shouldThrow<IOException> {
                OrtBomFileCurationProvider(curationsFile)
