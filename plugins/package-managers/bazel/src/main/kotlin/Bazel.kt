...
class Bazel(
    ...
    override fun resolveDependencies(
        ...
        var (mainModule, scopes) = getDependencyGraph(
            projectDir,
            depDirectives,
            archiveOverrides
        )
    ...
    private fun getPackages(
        ...
        // scopes are filtered to ignore the Conan scope.
        val ids = scopes.filterNot { it.name == CONAN_REQUIRES_SCOPE_NAME }.collectDependencies()
...
private fun AnalyzerConfiguration.getConanFactory() =
    determineEnabledPackageManagers()
        .filter { it.descriptor.id.startsWith("Conan") }
        .let { managers ->
            require(managers.size < 2) {
                "All of the $managers managers are able to manage 'Conan' projects. Please enable only one of them."
            }
