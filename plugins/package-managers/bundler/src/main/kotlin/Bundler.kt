...
class Bundler(
    ...
    override fun beforeResolution(
        ...
        if (config.bundlerVersion != null) {
            val duration = measureTime {
                val output = runScriptCode(
                    """
                    require 'rubygems/commands/install_command'
                    cmd = Gem::Commands::InstallCommand.new
                    cmd.handle_options ["--no-document", "--user-install", "$BUNDLER_GEM_NAME:${config.bundlerVersion}"]
                    cmd.execute
                    """.trimIndent()
                ).trim()
            ...
            logger.info { "Installing the '$BUNDLER_GEM_NAME' Gem in version ${config.bundlerVersion} took $duration." }
        }
