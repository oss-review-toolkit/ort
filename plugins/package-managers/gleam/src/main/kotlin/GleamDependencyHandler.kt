...
internal sealed interface GleamPackageInfo {
     ...
     */
    data class FromManifest(val pkg: GleamManifest.Package) : GleamPackageInfo {
        ...
        override fun toIdentifier(context: GleamProjectContext): Identifier =
            Identifier(
                type = if (pkg.source == GleamManifest.Package.SourceType.HEX) "Hex" else "OTP",
                namespace = "",
                name = pkg.name,
                version = pkg.version
            )
        ...
        override fun toOrtPackage(context: GleamProjectContext, issues: MutableCollection<Issue>): Package =
            when (pkg.source) {
                GleamManifest.Package.SourceType.HEX -> createHexPackage(context)
                GleamManifest.Package.SourceType.GIT -> createGitPackage(context)
                GleamManifest.Package.SourceType.LOCAL -> createLocalPackage(context, issues)
            }
     ...
     */
    data class FromDependency(val depName: String, val dep: GleamToml.Dependency) : GleamPackageInfo {
...
internal class GleamDependencyHandler : DependencyHandler<GleamPackageInfo> {
    ...
    private fun tryLock(dependency: GleamPackageInfo): GleamPackageInfo =
        when (dependency) {
            is GleamPackageInfo.FromManifest -> dependency
            is GleamPackageInfo.FromDependency -> manifestPackagesByName[dependency.name]?.let {
                GleamPackageInfo.FromManifest(it)
            } ?: dependency
        }

    override fun identifierFor(dependency: GleamPackageInfo): Identifier = tryLock(dependency).toIdentifier(context)

    override fun dependenciesFor(dependency: GleamPackageInfo): List<GleamPackageInfo> =
        tryLock(dependency).dependencies
            .mapNotNull { manifestPackagesByName[it] }
            .map { GleamPackageInfo.FromManifest(it) }
