...
* **All transitive dependencies** resolved and included (as a flat list)
...
wget https://downloads.apache.org/ant/ivy/2.5.3/apache-ivy-2.5.3-bin.tar.gz
tar -xzf apache-ivy-2.5.3-bin.tar.gz
...
4. **Dependency Tree Structure:**
    * Transitive resolution produces a flat list of all dependencies per scope
    * The parent-child hierarchy between transitive dependencies is not preserved
    * All resolved dependencies appear as direct children of each scope
...
| Dependency Depth | Direct only | All transitive (flat list) |
...
* Apache Ivy 2.5.3 is **pre-installed** in ORT container images (`ort:latest`)
