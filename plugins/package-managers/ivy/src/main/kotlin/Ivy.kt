 ...
 *
 * ## Features
 * - Transitive dependency resolution via Ivy CLI (disabled by default, opt-in)
 * - Parsing ivy.xml for direct dependencies (default mode)
 * - Support for multiple configurations (compile, runtime, test, etc.)
 * - Dynamic version resolution when using Ivy CLI
 *
 * ## Configuration
 * Transitive resolution is disabled by default. To enable it:
 * ```yaml
 * analyzer:
 *   package_managers:
 *     Ivy:
...
class Ivy(
    ...
    override fun resolveDependencies(
        ...
        val ivyModule = parseIvyModule(definitionFile) ?: return emptyList()
        ...
        val declaredLicenses = info.license?.name?.let { setOf(it) }.orEmpty()
        ...
        val project = Project(
            ...
            homepageUrl = info.description?.homepage.orEmpty(),
    ...
    private fun parseIvyModule(definitionFile: File): IvyModule? =
        try {
    ...
    private fun resolveDirectDependencies(
        ...
        val scopeMap = mutableMapOf<String, MutableSet<PackageReference>>()
        ...
        ivyModule.dependencies?.dependency?.forEach { dep ->
            ...
            scopeMap.getOrPut(scopeName) { mutableSetOf() }.add(packageRef)
            packages.add(createEmptyPackage(packageId))
        }

        val scopes = scopeMap.map { (name, deps) -> Scope(name = name, dependencies = deps) }.toSet()
