...
class MavenSupport(private val workspaceReader: WorkspaceReader) {
    companion object {
        ...
        internal fun RemoteRepository.toArtifactRepository(
            ...
            this@toArtifactRepository.proxy?.let { repoProxy ->
                proxy = Proxy().apply {
                    host = repoProxy.host
                    port = repoProxy.port
            ...
            this@toArtifactRepository.authentication?.let {
        ...
        private fun toMavenAuthentication(authContext: AuthenticationContext?): Authentication? =
            authContext?.let {
                Authentication(
                    it.get(AuthenticationContext.USERNAME),
                    it.get(AuthenticationContext.PASSWORD)
                ).apply {
                    passphrase = it.get(AuthenticationContext.PRIVATE_KEY_PASSPHRASE)
                    privateKey = it.get(AuthenticationContext.PRIVATE_KEY_PATH)
                }
            }
