...
class Bun(override val descriptor: PluginDescriptor = BunFactory.descriptor) :
    ...
    private val moduleInfoResolver = ModuleInfoResolver.create { workingDir, moduleId ->
        runCatching {
            val process = NpmCommand.DEFAULT.run(workingDir, "info", "--json", moduleId).requireSuccess()
    ...
    override fun beforeResolution(
        ...
        // NPM's listing machinery is used to resolve dependencies of Bun projects.
    ...
    private fun listModules(workingDir: File, issues: MutableList<Issue>): ModuleInfo {
        val listProcess = NpmCommand.DEFAULT.run(workingDir, "list", "--depth", "Infinity", "--json", "--long")
