...
class OrtBomFileFunTest : WordSpec({
    "resolveDependencies()" should {
        ...
        "return issue when json file is wrongly formatted" {
            val definitionFile = getAssetFile("projects/ort-bom-wrong-format.json")
        ...
        "return issue when package name is wrong" {
            val definitionFile = getAssetFile("projects/ort-bom-no-pkg-id-or-purl.yml")
            val project = OrtBomFileFactory.create().resolveSingleProject(definitionFile)
            project.packages.size shouldBe 0
            project.issues.size shouldBe 1
        ...
        "return issue when package has no id and purl" {
            val definitionFile = getAssetFile("projects/ort-bom-purl-and-id-for-same-pkg.yml")
            val project = OrtBomFileFactory.create().resolveSingleProject(definitionFile)
            project.packages.size shouldBe 1
            project.issues.size shouldBe 1

            with(project.issues[0]) {
                message shouldBe "There can be only one id or purl for package id:'Maven/com.example/full@1.1.0', " +
                    "curl:'pkg:maven/com.example/full@1.1.0'."
                source shouldBe "OrtBomFile"
            }
        }
