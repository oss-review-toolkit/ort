...
@OrtPlugin(
    ...
    description = "The package manager that uses ORT-specific file format as package list source.",
...
class OrtProjectFile(override val descriptor: PluginDescriptor = OrtProjectFileFactory.descriptor) :
    ...
    override fun resolveDependencies(
        ...
        try {
            parsedProject = when {
                definitionFile.name.endsWith(".json") ->
                    Json.decodeFromString<OrtProjectFileDto>(definitionFile.readText())

                definitionFile.name.endsWith(".yml") || definitionFile.name.endsWith(".yaml") ->
                    Yaml.default.decodeFromString<OrtProjectFileDto>(definitionFile.readText())

                else ->
                    error("Unknown file format for file '${definitionFile.absolutePath}'")
            }
        ...
        val project = OrtProjectFileMapper.extractAndMapProject(
            ...
            definitionFile.parentFile.relativeTo(analysisRoot).invariantSeparatorsPath
        )
