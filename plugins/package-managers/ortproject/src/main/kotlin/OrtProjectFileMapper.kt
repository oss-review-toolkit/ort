...
internal object OrtProjectFileMapper {
    ...
    internal fun extractAndMapProject(projectDto: OrtProjectFileDto, vcsInfo: VcsInfo, definitionFilePath: String) =
        Project(
            id = Identifier(
            ...
            declaredLicenses = projectDto.declaredLicenses.orEmpty(),
    ...
    private fun DependencyDto.getIdentifiers(): Pair<Identifier, String> {
        when {
            ...
            !purl.isNullOrBlank() ->
                ...
                error("There is something wrong in dependency id/purl declaration for '$this'.")
        }
    }
            ...
            if (dependency.scopes?.isNotEmpty() == true) {
                dependency.scopes.forEach { scopeName ->
                    if (!scopeMap.containsKey(scopeName)) {
                        scopeMap[scopeName] = mutableListOf()
                    }

                    scopeMap[scopeName]?.add(dependency.getIdentifiers().first)
                }
            }
        }

        return scopeMap.map { (scopeName, identifiers) ->
            Scope(
                name = scopeName,
