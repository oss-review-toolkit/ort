...
internal object OrtProjectFileMapper {
            ...
            val pkg = Package(
                ...
                sourceArtifact = sourceArtifact?.let { artifact ->
                    RemoteArtifact(
                        ...
                        hash = if (!artifact.hash.isNullOrBlank()) Hash.create(artifact.hash) else Hash.NONE
                    )
                }.orEmpty(),
                vcs = vcs.toVcsInfo(),
    ...
    private fun DependencyDto.validateIdentifiers() {
        if (!id.isNullOrBlank()) {
            ...
            require(!identifier.type.isBlank() && !identifier.name.isBlank() && !identifier.version.isBlank()) {
                throw IllegalArgumentException("The id '$id' is not a valid Identifier.")
            }
        }
    ...
    private fun VcsDto?.toVcsInfo(): VcsInfo =
        if (this == null) {
            ...
            VcsInfo(
                type = type.let { VcsType.forName(it) },
                url = url,
                revision = revision,
                path = path.orEmpty()
            )
