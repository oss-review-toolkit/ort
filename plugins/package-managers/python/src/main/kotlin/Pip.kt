...
internal val PYTHON_VERSIONS = listOf("2.7", "3.6", "3.7", "3.8", "3.9", "3.10", "3.11")
...
data class PipConfig(
    ...
    /**
     * The Python version to resolve dependencies for.
     */
    @OrtPluginOption(defaultValue = "3.11")
...
class Pip internal constructor(
    ...
    init {
        ...
        require(config.pythonVersion in PYTHON_VERSIONS) {
    ...
    internal fun runPythonInspector(
        ...
        logger.info {
            "Resolving dependencies for '${definitionFile.absolutePath}' with Python version " +
                "'${config.pythonVersion}' and operating system '${config.operatingSystem}'."
        }
        ...
        return runCatching {
            try {
                PythonInspector.inspect(
                    ...
                    pythonVersion = config.pythonVersion.split('.', limit = 3).take(2).joinToString(""),
