...
// npm ships with the Node.js distribution, under "lib/node_modules" everywhere but on Windows. Pick the
// layout from the OS like "nodeBinDir" above does, and not by probing the file system: this is evaluated
// at configuration time, before "kotlinNodeJsSetup" has downloaded Node.js, so on a clean checkout any
// probe finds nothing and silently selects the wrong layout.
val npmCliJs = if (Os.isFamily(Os.FAMILY_WINDOWS)) {
    nodeDir.resolve("node_modules/npm/bin/npm-cli.js")
} else {
    nodeDir.resolve("lib/node_modules/npm/bin/npm-cli.js")
}
