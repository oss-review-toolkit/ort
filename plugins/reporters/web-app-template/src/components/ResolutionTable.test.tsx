...
describe("ResolutionTable", () => {
    ...
    it("renders a row for a resolution", () => {
        // Build the row explicitly rather than reading it off the fixture: whether the embedded sample
        // model carries rule violation resolutions depends on which ORT result was imported.
        const message = "The license NOASSERTION is currently not covered by policy rules.";
        const resolution = new WebAppResolution({ _id: 0, message, reason: "CANT_FIX_EXCEPTION" });

        const { container } = render(<ResolutionTable resolutions={[resolution]} />);
        expect(container.textContent).toContain(message);
        expect(container.textContent).toContain("CANT_FIX_EXCEPTION");
    });

    it("renders the sample model's resolutions without falling over", () => {
        const { container } = render(<ResolutionTable resolutions={resolutions} />);
