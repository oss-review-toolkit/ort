...
describe("ScopeExcludesTable", () => {
    let scopeExcludes: readonly WebAppScopeExclude[];

    beforeAll(async () => {
        const result = await buildResult(loadSampleEvaluatedModel());
        scopeExcludes = result.scopeExcludes;
    });
    ...
    it("shows the empty state for the sample model, which has no scope excludes", () => {
        render(<ScopeExcludesTable scopeExcludes={scopeExcludes} />);
        expect(screen.getByText("No scope excludes")).toBeInTheDocument();
    });

    it("shows the empty state when there are no scope excludes", () => {
        render(<ScopeExcludesTable scopeExcludes={[]} />);
        expect(screen.getByText("No scope excludes")).toBeInTheDocument();
    });
});
