...
function AppPage({ webAppEvaluatedModel }: AppPageProps): JSX.Element {
    ...
    // Jump to the Packages tab, search for the given package id and expand its row.
    ...
    // Jump to the Packages tab and filter the given license column on the given license.
    ...
    // Jump to the Packages tab, optionally pre-filtered on the given dependency kinds (null = unfiltered).
    ...
    // A view (Packages/Dependency Graph) reports which package is expanded/selected so the URL can deep-link it;
    ...
    const nav: { id: TabId; label: string; icon: LucideIcon; color: string; count?: number }[] = [
        ...
        { id: "table", label: "Packages", icon: TableIcon, color: "text-[#0E8079]" },
        { id: "tree", label: "Dependency Graph", icon: Network, color: "text-[#3A8DCC]" },
