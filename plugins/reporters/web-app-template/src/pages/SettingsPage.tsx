...
import { convertIso8601Date2Sentence } from "@/lib/dates";
...
function isColumnPresent(column: ResultsTableColumn, model: WebAppEvaluatedModel): boolean {
    switch (column.presence) {
        ...
        case "published":
            return model.hasPublishedDates();
        default:
            return true;
    }
}
...
function SettingsPage({ webAppEvaluatedModel }: SettingsPageProps): JSX.Element {
    ...
    // A late-night UTC instant, so the preview visibly changes the calendar date for readers west of UTC
    // rather than only the wording.
    const example = convertIso8601Date2Sentence("2022-06-29T01:52:10Z", dateFormat);
        ...
        <div className="mx-auto flex max-w-4xl flex-col gap-4">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Timestamps are recorded as UTC instants. By default they are shown in UTC using US English, so
                        the same report reads the same for everyone who opens it — a package published at 01:52 UTC is
                        the day before in the Americas, which would otherwise make readers disagree about its release
                        date. Turn this on to read dates in your own language and timezone instead.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="flex items-center justify-between gap-4 text-sm">
                        <label className="cursor-pointer" htmlFor="browser-date-format">
                            Use my browser's language and timezone
                        </label>
                        <Switch
                            checked={settings.useBrowserDateFormat}
                            id="browser-date-format"
                            onCheckedChange={(value) => updateSettings({ useBrowserDateFormat: value })}
                        />
                    </div>
                    <p className="mt-3 text-muted-foreground text-xs">Example: {example}</p>
                </CardContent>
            </Card>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>Default packages table columns</CardTitle>
                    <CardDescription>
                        Choose which columns are shown by default in the Packages view. You can still show or hide any
