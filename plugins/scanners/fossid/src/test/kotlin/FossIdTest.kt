    ...
    "scanPackages()" should {
        ...
        "create a new scan for an existing project" {
            ...
            val service = FossIdRestService.create(config.serverUrl)
                .expectCloneRepositoryWorkflow(projectCode, scanCode, vcsInfo, branchName, listOf(scan))
                .mockFiles(scanCode)
            ...
            val comment = createOrtScanComment(vcsInfo.url, vcsInfo.revision, branchName)
            service.verifyCloneRepositoryWorkflow(projectCode, scanCode, vcsInfo, comment)
        }
        ...
        "create a new scan for an existing project by uploading an archive" {
            ...
            val comment = createOrtScanComment(vcsInfo.url, vcsInfo.revision, branchName)
            service.verifyUploadArchiveWorkflow(projectCode, scanCode, comment = comment)
        }
        ...
        "rewrite the snippet results paths for a scan created by uploading an archive" {
            ...
            val comment = createOrtScanComment(vcsInfo.url, vcsInfo.revision, branchName)
            service.verifyUploadArchiveWorkflow(projectCode, scanCode, comment)
        ...
        "throw an exception if the scan download failed" {
            ...
            val comment = createOrtScanComment(vcsInfo.url, vcsInfo.revision, REVISION_EMPTY)
            service.verifyCloneRepositoryWorkflow(projectCode, scanCode, vcsInfo, comment, verifyDeleteScan = true)
        }
        ...
        "return copyright and license findings from a new scan" {
            ...
            FossIdRestService.create(config.serverUrl)
                .expectCloneRepositoryWorkflow(projectCode, scanCode, vcsInfo, REVISION_EMPTY, listOf(scan))
        ...
        "take ignored files into account" {
            ...
            FossIdRestService.create(config.serverUrl)
                .expectCloneRepositoryWorkflow(projectCode, scanCode, vcsInfo, REVISION_EMPTY, emptyList())
        ...
        "explicitly start a scan if necessary" {
            ...
            val service = FossIdRestService.create(config.serverUrl)
                .expectCloneRepositoryWorkflow(
                    projectCode,
                    scanCode,
                    vcsInfo,
                    REVISION_EMPTY,
                    checkScanStatuses = listOf(ScanStatus.NOT_STARTED, ScanStatus.FINISHED)
                )
                .mockFiles(scanCode)
            coEvery {
                service.runScan(
        ...
        "wait for a scan to complete" {
            ...
            val service = FossIdRestService.create(config.serverUrl)
                .expectCloneRepositoryWorkflow(
                    ...
                    vcsInfo,
                    REVISION_EMPTY,
                    checkScanStatuses = listOf(ScanStatus.NOT_STARTED, ScanStatus.SCANNING, ScanStatus.FINISHED)
                )
                .mockFiles(scanCode)
            coEvery {
                service.runScan(
        ...
        "create a delta scan for an existing scan" {
            ...
            val comment = createOrtScanComment(vcsInfo.url, vcsInfo.revision, REVISION_MASTER)
            service.verifyCloneRepositoryWorkflow(
                projectCode, scanCode, vcsInfo, comment,
                mapOf(
        ...
        "create a delta scan for a branch (fallback to default branch)" {
            ...
            val service = FossIdRestService.create(config.serverUrl)
                .expectCloneRepositoryWorkflow(projectCode, scanCode, vcsInfo, branchName, listOf(scan, scan2), true)
                .mockFiles(scanCode)
            coEvery { service.runScan(any()) } returns EntityResponseBody(status = 1)
        ...
        "carry exclusion rules to a delta scan from an existing scan (legacy behavior)" {
            ...
            val service = FossIdRestService.create(config.serverUrl)
                .expectCloneRepositoryWorkflow(
                    ...
                    existingScans = listOf(scan),
                    deltaScans = true,
                    existingIgnoreRules = listOf(IGNORE_RULE)
                )
            ...
            service.verifyCloneRepositoryWorkflow(
                ...
                ignoreRulesScancode = originCode
            )
        }
        ...
        "create an origin scan if no scan exists yet" {
            ...
            val service = FossIdRestService.create(config.serverUrl)
                .expectCloneRepositoryWorkflow(projectCode, scanCode, vcsInfo, REVISION_EMPTY, deltaScans = true)
                .mockFiles(scanCode, identifiedRange = 1..2, markedRange = 1..2)
            coEvery { service.runScan(any()) } returns EntityResponseBody(status = 1)
        ...
        "delete newly triggered scans if a package cannot be scanned" {
            ...
            val service = FossIdRestService.create(config.serverUrl)
                .expectCloneRepositoryWorkflow(projectCode, scanCode1, vcsInfo1, REVISION_EMPTY)
            ...
            service.expectProjectRequest(project3)
                .expectCloneRepositoryWorkflow(project3, scanCode3, vcsInfo3, REVISION_EMPTY)
        ...
        "enforce a limit on the number of delta scans" {
            ...
            val service = FossIdRestService.create(config.serverUrl)
                .expectCloneRepositoryWorkflow(
                    ...
                    scans,
                    true,
                    existingScanCode = recentScan.code!!
                )
                .mockFiles(scanCode)
            coEvery { service.runScan(any()) } returns EntityResponseBody(status = 1)
            coEvery { service.deleteScan(any()) } returns EntityResponseBody(status = 1)
