...
class ScanOssScannerDirectoryTest : StringSpec({
    ...
    "Scanner should exclude only files matching the specified path pattern (**/*.kt)" {
        ...
        // Verify our test file exists - this file should be included in the scan
        // since it doesn't match the exclusion pattern (it's a .go file, not .kt)
        val includedFile = File(EXCLUSION_TEST_DIRECTORY, "server.go")
        if (!includedFile.exists()) {
            fail("file ${includedFile.absolutePath} does not exist")
        }

        // Run the scanner with our exclusion pattern
        // This will traverse the directory and should skip .kt files
        scanner.scanPath(
        ...
        //
        // file=[hash],[size],[filename]  <-- This is the format we're extracting with regex
