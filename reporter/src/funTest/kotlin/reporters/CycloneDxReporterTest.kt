...
class CycloneDxReporterTest : WordSpec({
    "A generated BOM" should {
        "be valid" {
            ...
            val bomFile = CycloneDxReporter().generateReport(ReporterInput(ORT_RESULT), outputDir).single()
        ...
        "match the result from the official Gradle plugin" {
            ...
            val outputDir = createTempDir(ORT_NAME, javaClass.simpleName).apply { deleteOnExit() }
            val bomFileFromReporter = CycloneDxReporter().generateReport(ReporterInput(ortResult), outputDir).single()
