...
private fun String.removeDuplicateWhitespaces() =
        replace("(?=.*)([ ]+)".toRegex(), " ")

private fun String.removeTrailingSpecialCharacters() =
        replace("^([<> \\(\\)\\[\\];\\.,\\-\\+\\~&]*)".toRegex(), "")
...
private fun Collection<Parts>.groupByPrefixAndOwner(): List<Parts> {
    ...
    forEach { part ->
        val key = "${part.prefix}:${part.owner.removeSpecialCharacters().toUpperCase()}"
        if (!map.containsKey(key)) {
            ...
            map[key] = Parts(
                    prefix = other.prefix,
                    years = part.years + other.years,
                    owner = other.owner,
                    originalStatements = part.originalStatements + other.originalStatements
            )
        }
    }
...
class CopyrightStatementsProcessor {
    ...
    data class Result(val processedStatements: LinkedHashMap<String, List<String>>,
        ...
        fun toList() = unprocessedStatements + processedStatements.keys
    ...
    fun process(vararg copyrightStatements: String) =
            process(copyrightStatements.asList())

    fun process(copyrightStatments: Collection<String>): Result {
        val unprocessedStatements = copyrightStatments.filter { copyrightStatement ->
            determineParts(copyrightStatement) == null
        }

        val parts = copyrightStatments.mapNotNull { copyrightStatement ->
            determineParts(copyrightStatement)
        }.groupByPrefixAndOwner().sortedWith(
                compareBy(
                        { it.owner },
        ...
        parts.forEach {
            if (!it.owner.isEmpty()) {
                val statement = with(StringBuilder()) {
                    append(it.prefix)
                    if (!it.years.isEmpty()) {
                        append(" ")
    ...
    private fun stripKnownCopyrightPrefix(copyrightStatement: String): Pair<String, String> {
        KNOWN_PREFIX_REGEX.forEach { regex ->
            regex.find(copyrightStatement)?.let { matchResult ->
                return Pair(
                        first = copyrightStatement.replace(matchResult.groups[1]!!.value, ""),
    ...
    private fun replaceYears(copyrightStatement: String, placeholder: String): Pair<String, Set<Int>> {
        ...
        resultYears.addAll(replaceRangeResult.second)
        ...
        while (matchResult != null) {
            ...
            resultYears.add(matchResult.groups[1]!!.value.toInt())
