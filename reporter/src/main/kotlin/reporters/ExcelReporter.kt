...
class ExcelReporter : TableReporter() {
    ...
    override fun generateReport(tableizedScanRecord: TableizedScanRecord, outputDir: File) {
        ...
        successStyle = workbook.createCellStyle().apply {
            cloneStyleFrom(defaultStyle)
            setFillForegroundColor(XSSFColor(Color(173, 216, 230)))
            setFillPattern(FillPatternType.SOLID_FOREGROUND)
        }

        warningStyle = workbook.createCellStyle().apply {
            cloneStyleFrom(defaultStyle)
            setFillForegroundColor(XSSFColor(Color(255, 255, 224)))
            setFillPattern(FillPatternType.SOLID_FOREGROUND)
        }

        errorStyle = workbook.createCellStyle().apply {
            cloneStyleFrom(defaultStyle)
            setFillForegroundColor(XSSFColor(Color(240, 128, 128)))
            setFillPattern(FillPatternType.SOLID_FOREGROUND)
        }

        createSheet(workbook, "Summary", "all", tableizedScanRecord.summary)
        tableizedScanRecord.projectDependencies.forEach { project, table ->
