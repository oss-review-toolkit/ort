...
class StaticHtmlReporter : Reporter() {
    ...
    private fun TBODY.issueRow(rowIndex: Int, row: ReportTableModel.IssueRow) {
        ...
        val worstSeverity = row.analyzerIssues.map { (_, issues) ->
            issues.map { it.severity }.min() ?: Severity.ERROR
        }.min() ?: Severity.ERROR

        val areAllResolved = row.analyzerIssues.isNotEmpty() && row.analyzerIssues.map { (_, issues) ->
