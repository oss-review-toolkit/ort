...
class AOSD2JSONReport : Reporter {
    ...
    override fun generateReport(input: ReporterInput, outputDir: File, options: Map<String, String>): List<File> {
        ...
        val navigator = DependencyGraphNavigator(input.ortResult)

        for (project in input.ortResult.getProjects()) {
            navigator.scopeNames(project).forEach { scope ->
                combinedDirectDependencies += navigator.dependenciesForScope(project, scope, 1)
    ...
    private fun getLicenseFindings(input: ReporterInput, project: Project): Map<Identifier, AOSD2Dependency> =
        input.ortResult.collectDependencies(project.id)
            ...
            .associateWith { id ->
                ...
                val artifact = pkg.pkg.sourceArtifact.takeUnless {
                    it == RemoteArtifact.EMPTY
                ...
                AOSD2Dependency(
                    ...
                    listOf(AOSD2Part()),
                    deployPackage = artifact.aosd2DeployPackage(),
                    dependencies
                )
            }

    private fun Identifier.nameWithOptionalNamespace(): String =
        if (namespace.isEmpty()) name else listOf(namespace, name)
            .joinToString("/")
    ...
    private fun CuratedPackage.toAOSD2Licenses(
        ...
        return licenses.map { license ->
            val curatedCopyrightFindings = license.getResolvedCopyrights()
                .flatMap { it.findings }
                .filter { it.copyrightSource == ResolvedCopyrightSource.PROVIDED_BY_CURATION }
                .mapTo(mutableSetOf()) { it.statement }
            val scannedCopyrightFindings = license.getResolvedCopyrights()
                .flatMap { it.findings }
            ...
            AOSD2License(
                ...
                AOSD2Copyright(
                    curatedCopyrightFindings.ifEmpty { scannedCopyrightFindings },
                    String.format(Locale.GERMAN, "Author: %s", pkg.authors)
                ),
