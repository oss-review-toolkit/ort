...
[#-- Function to extract a human-readable package name from its identifier. --]
[#function packageName package]
  [#local namespace = ""]
  [#if package.id.namespace?has_content]
    [#local namespace = package.id.namespace + ":"]
  [/#if]
  [#return namespace + package.id.name + ":" + package.id.version]
[/#function]
...
License File: <<${packageName(package)} ${licenseFile.path}, ${licenseFile.path}>>
...
[#list package.licenseFiles.files as licenseFile]
=== ${packageName(package)} ${licenseFile.path}
