...
class SummaryView extends React.Component {
    ...
    constructor(props) {
        ...
        if (props.reportData) {
            ...
            if (this.state.data.licenses 
                && this.state.data.licenses.data 
                && this.state.data.licenses.total) {
                if (this.state.data.licenses.data.detected 
                    && this.state.data.licenses.total.detected) {
                ...
                if (this.state.data.licenses.data.declared 
                    ...
                    viewData.charts.declaredLicenses = this.convertLicensesToChartFormat(
                        this.state.data.licenses.data.declared);
            ...
            if (this.state.data.errors && this.state.data.errors.data && this.state.data.errors.total) {
                if (this.state.data.errors.data.open && this.state.data.errors.total.open) {
                ...
                if (this.state.data.errors.data.addressed && this.state.data.errors.total.addressed) {
                    viewData.errors.addressed = this.convertErrorsToTableFormat(this.state.data.errors.data.addressed);
                    viewData.errors.totalAddressed = this.state.data.errors.total.addressed;
                }
            }
        }
        ...
        const SummaryErrors = () => {
                ...
                if (viewData.errors.totalOpen !== 0) {
                    return (
                        <Tabs tabPosition="top">
                            ...
                            <TabPane tab={<span>Addressed Errors ({viewData.errors.totalAddressed})</span>} key="2">
                                {renderErrorTable(viewData.errors.addressed, viewData.errors.totalAddressed)}
                            </TabPane>
                        </Tabs>
                    );
                }
            ...
            const SummaryTimeline = () => {
                ...
                const renderCompletedText = () => {
                    if (nrErrors !== 0) {
                        return (<span style={{color: '#f5222d', fontSize: 18, lineHeight: '1.2'}}>
                            <b>Completed scan with {nrErrors} errors </b>
                        </span>);
                    }

                    return (<span style={{color : '#52c41a', 'fontSize': 18, lineHeight: '1.2'}}>
                        <b>Completed scan</b>
                    </span>);
                };
                let vcs;
                ...
                if (data && data.vcs && data.vcs_processed) {
                    ...
                    return (
                        <Timeline>
                            ...
                            <Timeline.Item dot={<Icon type={(nrErrors !== 0) ? 'exclamation-circle-o' : 'check-circle-o'} style={{ fontSize: 16 }} />} 
