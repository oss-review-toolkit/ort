...
object Main {
    ...
    private fun scanInputPath(inputPath: File): ScanRecord {
        ...
        val result = try {
            ...
            val summary = ScanSummary(Instant.now(), Instant.now(), 0, sortedSetOf(), e.collectMessages().toSortedSet())
            ScanResult(Provenance(Instant.now()), localScanner.getDetails(), summary)
        }
    ...
    private fun writeScanRecord(outputDirectory: File, scanRecord: ScanRecord) {
        summaryFormats.forEach { format ->
            val summaryFile = File(outputDirectory, "scan-record.${format.fileExtension}")
            val mapper = when (format) {
                OutputFormat.JSON -> jsonMapper
                OutputFormat.YAML -> yamlMapper
            }
            println("Writing scan record to ${summaryFile.absolutePath}.")
            mapper.writerWithDefaultPrettyPrinter().writeValue(summaryFile, scanRecord)
        }
    }
}
