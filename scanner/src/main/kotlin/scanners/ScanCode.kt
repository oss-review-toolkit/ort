 ...
 * * "commandLineNonMod": Command line options that do not modify the result, like "--processes". Default to
 ...
 * * "debugCommandLine": Debug command line options that do not modify the result, only used if the [log] level is set
 *   to [Level.DEBUG]. Defaults to [DEFAULT_DEBUG_NON_CONFIGURATION_OPTIONS].
 */
class ScanCode(config: ScannerConfiguration) : LocalScanner(config) {
    ...
    private val nonConfigurationOptions = scanCodeConfiguration["commandLineNonMod"]?.split(" ")
            ?: DEFAULT_NON_CONFIGURATION_OPTIONS
    private val debugConfigurationOptions = scanCodeConfiguration["debugCommandLine"]?.split(" ")
            ?: DEFAULT_DEBUG_CONFIGURATION_OPTIONS
    private val debugNonConfigurationOptions = scanCodeConfiguration["debugCommandLineNonMod"]?.split(" ")
