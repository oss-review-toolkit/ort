...
fun Component.toIdentifier(defaultType: String = DEFAULT_ORT_TYPE): Identifier {
    ...
    return Identifier(
        type = type,
        namespace = parsedPurl?.namespace?.ifBlank { null } ?: group.orEmpty(),
        name = parsedPurl?.name?.ifBlank { null } ?: name,
        version = parsedPurl?.version?.ifBlank { null } ?: version.orEmpty()
    )
}
...
internal fun Component.extractSourceArtifact(): RemoteArtifact {
    ...
    val hashes = ref.hashes?.mapNotNull { cdxHash ->
        mapHashAlgorithm(cdxHash.algorithm).takeIf { it.isVerifiable }?.let { Hash(cdxHash.value, it) }
    }
