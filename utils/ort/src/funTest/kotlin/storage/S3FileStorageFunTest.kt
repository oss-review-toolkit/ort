...
class S3FileStorageFunTest : WordSpec() {
    ...
    private val handler = object : HttpHandler {
        val requests = Collections.synchronizedMap(mutableMapOf<String, String>())
        ...
        override fun handle(exchange: HttpExchange) {
            when (exchange.requestMethod) {
                ...
                "PUT" -> {
                    val key = exchange.requestURI.toString().removeSurrounding("/$bucket/", "?x-id=PutObject")
                    requests[key] = exchange.requestBody.reader().use { it.readText() }
                    exchange.sendResponseHeaders(HttpURLConnection.HTTP_OK, 0)
                }
                "GET" -> {
                    val key = exchange.requestURI.toString().removeSurrounding("/$bucket/", "?x-id=GetObject")
                    requests[key].let { data ->
                        if (data != null) {
                            exchange.sendResponseHeaders(HttpURLConnection.HTTP_OK, 0)
